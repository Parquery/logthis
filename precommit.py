...
def main() -> int:
    ...
    # yapf: disable
    source_files = (
                sorted((repo_root / "logthis").glob("**/*.py")) +
    ...
    print("YAPF'ing...")
    yapf_targets = ["tests", "logthis", "setup.py", "precommit.py"]
    if overwrite:
        # yapf: disable
    ...
    print("Mypy'ing...")
    subprocess.check_call(["mypy", "--strict", "logthis", "tests"], cwd=str(repo_root))
    ...
    # yapf: disable
    subprocess.check_call(
        ["isort", "--project", "logthis", '--line-width', '120'] +
        ([] if overwrite else ['--check-only']) +
        [str(pth) for pth in source_files])
    # yapf: enable

    print("Pydocstyle'ing...")
    subprocess.check_call(["pydocstyle", "logthis"], cwd=str(repo_root))

    print("Pylint'ing...")
    subprocess.check_call(["pylint", "--rcfile=pylint.rc", "tests", "logthis"], cwd=str(repo_root))
    ...
    print("Doctesting...")
    doctest_files = ([repo_root / "README.rst"] + sorted((repo_root / "logthis").glob("**/*.py")))
